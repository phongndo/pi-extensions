...
A focused local extension suite for [Pi](https://github.com/badlogic/pi-mono): native interactive clarification, faster Codex requests, durable notes and historical recall, bounded web access, a native MCP server menu, plain-language restatements, visual explanations, plan stress-testing, repo-native domain docs, multi-session planning, stateful teaching, session handoffs, safe PR publishing, and human-invoked PR autopilot.

The workspace is one Pi package, so installation exposes every extension, the bundled skills, and the `origin` theme together. This repository owns the complete Pi package: extensions, commands, theme, and canonical skills. Pi loads those skills directly; they are not generated by chezmoi. [nix-config's chezmoi setup](https://github.com/phongndo/nixos-config/blob/main/docs/agent-skills.md) reads the skill library to install compatible copies for other agents, without writing into this checkout.
...
| [Context](extensions/context/README.md)     | Long tasks need durable notes and recoverable evidence                  | `/context on\|off\|status`, `recall`, `notes` | Fresh-window resets default on; normal compaction fallback      |
...
| [MCP](extensions/mcp/README.md)             | You want Pi to use MCP servers already configured for other agents      | `/mcp`                                        | Native tools with a minimal footer `mcp connected/total` status |
...
Eligible `openai-codex` requests gain `service_tier: "priority"`; Pi's native footer status shows `speed fast` while enabled and supported, and hides it when off.
...
Fresh-window resets are enabled by default. The agent saves notes and a verified checkpoint before dropping old active messages, then retrieves missing evidence with `recall`. Off restores normal Pi compaction without deleting notes or disabling retrieval. See [Context](extensions/context/README.md) for safeguards and limitations.
...
| Context       | `~/.pi/agent/context.json`; Pi session JSONL | Global on/off preference; branch-local notes, checkpoints and evidence |
...
│   ├── question/                   # question
│   ├── fast-mode/                  # /fast
│   ├── context/                    # /context, recall, notes, verified fresh windows
...
- [Context](extensions/context/README.md)
