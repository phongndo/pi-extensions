...
Agent options are { tools, model, thinking, retries, timeoutMs }. Tools are selected from read, grep, find, ls, edit, write, bash. Model must be an exact provider/model-id from the live catalog supplied with the task; thinking must be supported by that model. Omit either field to inherit the outer session choice. Always specify the smallest tool list. Read-only tasks may retry at most twice. Mutation or shell tasks are never retried by the host because their effects may not be idempotent. Source review authorizes the declared workflow, so routine project edits and verification should proceed autonomously without $.approval. Add a runtime approval only when the user's goal explicitly requests one or before an irreversible external side effect. Use bash only when verification truly requires it.
