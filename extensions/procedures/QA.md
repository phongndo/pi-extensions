...
3. Run autonomously after source review: do not add a runtime approval checkpoint for routine file mutation or verification. Log the proposed files, model allocation, and safety boundary before implementation.
4. Have an implementation agent create the intentionally bad first version. It should have several realistic defects: weak input validation, collision-prone IDs, incorrect status codes, unsafe persistence assumptions, and inadequate tests. Keep it dependency-free so no network install is needed.
...
5. Close `/monitor` and confirm the main session remains usable while only a compact footer status is shown, with no persistent progress widget below the editor.
...
11. Start another run and press `x` to verify cancellation preserves completed edits and records a cancelled snapshot; then press `r` on the terminal run (or use `/proc restart <run-id>`) and verify a new run starts from retained source.
