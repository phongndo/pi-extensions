...
The footer shows `Account: Personal` (or the default account name without an alias) for the selected subscription provider. Routed requests update it immediately on account selection, including fallback; before the first request it shows the highest-ranked account not on cooldown. Emails and credentials are never displayed there.

There are no Router add/remove/login commands, modes, or separate secret prompts. Single-account providers do not appear in the ranking list. With none eligible, `/router` gives a short notice rather than an empty panel. Rankings are **within a provider**, never across providers.
